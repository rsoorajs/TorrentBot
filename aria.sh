...
ARIA_RPC_SECRET="hasdhashdasndashdagnausdasmd"
...
aria2c --bt-tracker=https://raw.githubusercontent.com/cikgusuraya/KillerAdBlok/master/tracker.txt --bt-max-peers=0  --enable-rpc --rpc-listen-all=false --rpc-listen-port $RPC_LISTEN_PORT --max-concurrent-downloads=$MAX_CONCURRENT_DOWNLOADS --max-connection-per-server=10 --rpc-max-request-size=1024M --seed-time=0.01 --min-split-size=10M --follow-torrent=mem --split=10 --rpc-secret=$ARIA_RPC_SECRET --max-overall-upload-limit=1  --daemon=true
